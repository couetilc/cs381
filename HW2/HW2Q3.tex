...
\begin{quote}
    ...
    \begin{steps}
        ...
        \item $b = k[0]$
        \item if ($b$ is a disjunctive bid or a bid for ham) and $h \geq b_y$
        \begin{steps}
            \item Return $Max($
            \\ $b_x - \texttt{cost($b_y$)} + \texttt{subs($k \setminus b$, $h-b_y$, $v$)}$, \textcolor{ForestGreen}{// Take the bid}
            \\ $\texttt{subs($k \setminus b$, $h$, $v$)}$ \textcolor{ForestGreen}{// Don't take the bid}
            \\ $)$
        \end{steps}
        \item if ($b$ is a disjunctive bid or a bid for veggie) and $v \geq b_y$
        \begin{steps}
            \item Return $Max($
            \\ $b_x - \texttt{cost($b_y$)} + \texttt{subs($k \setminus b$, $h$, $v-b_y$)}$, \textcolor{ForestGreen}{// Take the bid}
            \\ $\texttt{subs($k \setminus b$, $h$, $v$)}$ \textcolor{ForestGreen}{// Don't take the bid}
            \\ $)$
        \end{steps}
